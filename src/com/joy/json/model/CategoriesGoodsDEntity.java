package com.joy.json.model;

import java.util.List;

import android.widget.BaseAdapter;

public class CategoriesGoodsDEntity extends TResult{
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	
	private String flag;
	private String categoryId;
	private String msg;
	private List<CategoriesGoods> retobj;
	
	public String getFlag() {
		return flag;
	}

	public void setFlag(String flag) {
		this.flag = flag;
	}
	
	public String getCategoryId() {
		return categoryId;
	}

	public void setCategoryId(String categoryId) {
		this.categoryId = categoryId;
	}

	public String getMsg() {
		return msg;
	}

	public void setMsg(String msg) {
		this.msg = msg;
	}
	
	public List<CategoriesGoods> getRetobj() {
		return retobj;
	}

	public void setRetobj(List<CategoriesGoods> retobj) {
		this.retobj = retobj;
	}
	
	public class CategoriesGoods extends TResult{
		int Id;
		String ComNo;
		String ComName;
		String ComDesc;
		String AppComDesc;
		String Picture;
		String AppPicture;
		int ComtCategory1;
		int ComtCategory2;
		int ComBrand;
		int Points;
		int MarketPrice;
		int ComOrderSx;
		String Flag;
		String CreateTime;
		String SpecialHint;
		String Service;
		String CategoryName;
		String BrandName;
		int ComCount;
		public int getId() {
			return Id;
		}
		public void setId(int id) {
			Id = id;
		}
		public String getComNo() {
			return ComNo;
		}
		public void setComNo(String comNo) {
			ComNo = comNo;
		}
		public String getComName() {
			return ComName;
		}
		public void setComName(String comName) {
			ComName = comName;
		}
		public String getComDesc() {
			return ComDesc;
		}
		public void setComDesc(String comDesc) {
			ComDesc = comDesc;
		}
		public String getAppComDesc() {
			return AppComDesc;
		}
		public void setAppComDesc(String appComDesc) {
			AppComDesc = appComDesc;
		}
		public String getPicture() {
			return Picture;
		}
		public void setPicture(String picture) {
			Picture = picture;
		}
		public String getAppPicture() {
			return AppPicture;
		}
		public void setAppPicture(String appPicture) {
			AppPicture = appPicture;
		}
		public int getComtCategory1() {
			return ComtCategory1;
		}
		public void setComtCategory1(int comtCategory1) {
			ComtCategory1 = comtCategory1;
		}
		public int getComtCategory2() {
			return ComtCategory2;
		}
		public void setComtCategory2(int comtCategory2) {
			ComtCategory2 = comtCategory2;
		}
		public int getComBrand() {
			return ComBrand;
		}
		public void setComBrand(int comBrand) {
			ComBrand = comBrand;
		}
		public int getPoints() {
			return Points;
		}
		public void setPoints(int points) {
			Points = points;
		}
		public int getMarketPrice() {
			return MarketPrice;
		}
		public void setMarketPrice(int marketPrice) {
			MarketPrice = marketPrice;
		}
		public int getComOrderSx() {
			return ComOrderSx;
		}
		public void setComOrderSx(int comOrderSx) {
			ComOrderSx = comOrderSx;
		}
		public String getFlag() {
			return Flag;
		}
		public void setFlag(String flag) {
			Flag = flag;
		}
		public String getCreateTime() {
			return CreateTime;
		}
		public void setCreateTime(String createTime) {
			CreateTime = createTime;
		}
		public String getSpecialHint() {
			return SpecialHint;
		}
		public void setSpecialHint(String specialHint) {
			SpecialHint = specialHint;
		}
		public String getService() {
			return Service;
		}
		public void setService(String service) {
			Service = service;
		}
		public String getCategoryName() {
			return CategoryName;
		}
		public void setCategoryName(String categoryName) {
			CategoryName = categoryName;
		}
		public String getBrandName() {
			return BrandName;
		}
		public void setBrandName(String brandName) {
			BrandName = brandName;
		}
		public int getComCount() {
			return ComCount;
		}
		public void setComCount(int comCount) {
			ComCount = comCount;
		}
		
		
	}
}
