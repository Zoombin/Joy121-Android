...
import com.joy.Fragment.PortalsFragment.ModulesAdapter;
...
public class PersonalFragment extends BaseFragment implements OnClickListener {
	...
	private RelativeLayout layout_isMobileBind;
	private TextView tv_isMobileBind;
	private TextView tv_isMobileBindInfo;//显示是否已经绑定手机
	
	private TextView tv_bindMobile;//弹出对话框显示手机号码
	...
	private ImageView img_help_arrow;
	...
	public void onResume() {
		...
		if (appSet != null) {
			int imgid = 0;
			try {
				imgid = ResName2ID.getDrawableID(mActivity, appSet.getLogo()
						.replaceAll(".png", ""));
			} catch (Exception e) {
			...
			/*
			 * if(imgid != 0){ tv_title.setVisibility(View.GONE);
			...
			ImageLoader.getInstance().displayImage(
					Constants.IMGLOGO + appSet.getLogo(), ivLogo, options);
	...
	private void initView(View v) {
		...
		uiAdapter.setMargin(tv_company, LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT, 0, 2, 0, 0);
		...
		uiAdapter.setMargin(tv_points_title, LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT, 0, 2, 0, 20);
		...
		uiAdapter.setMargin(layout_personalinfo, LayoutParams.MATCH_PARENT, 54,
				0, 0, 0, 0);
		...
		uiAdapter.setMargin(tv_personalinfo, LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT, 64, 0, 0, 0);
		...
		layout_isMobileBind = (RelativeLayout) v
				.findViewById(R.id.layout_isMobileBind);
		uiAdapter.setMargin(layout_isMobileBind, LayoutParams.MATCH_PARENT, 54,
				0, 0, 0, 0);
		layout_isMobileBind.setOnClickListener(this);
		tv_isMobileBind = (TextView) v.findViewById(R.id.tv_isMobileBind);
		uiAdapter.setTextSize(tv_isMobileBind, 20);
		uiAdapter.setMargin(tv_isMobileBind, LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT, 64, 0, 20, 0);
		tv_isMobileBindInfo = (TextView) v
				.findViewById(R.id.tv_isMobileBindInfo);
		uiAdapter.setTextSize(tv_isMobileBindInfo, 18);
		uiAdapter.setMargin(tv_isMobileBindInfo, 60,
				uiAdapter.CalcHeight(12, 16, 28), 0, 0, 20, 0);
		...
		uiAdapter.setMargin(btn_loginout, LayoutParams.MATCH_PARENT, 46, 15, 0,
				15, 50);
		...
		if (color != 0) {
			...
			GradientDrawable shapeDrawable = (GradientDrawable) btn_loginout
					.getBackground();
	...
	//刷新信息已绑定
	public void setBindMobileString(String str) {
		tv_isMobileBindInfo.setText(str);
		tv_isMobileBindInfo.setTextColor(Color.parseColor("#006600"));
	}
		...
		OnOperationListener listener = new OnOperationListener() {
			...
			public void onOperationFinished(List<Object> resList) {
				...
				if (userinfo != null) {
					...
					if(userinfo.getIsMobileBind()==1)//绑定手机信息
					{
						tv_isMobileBindInfo.setText("已绑定");
						tv_isMobileBindInfo.setTextColor(Color.parseColor("#006600"));
					}else{
						tv_isMobileBindInfo.setText("未绑定");
						tv_isMobileBindInfo.setTextColor(Color.parseColor("#FF0000"));
						new Handler().postDelayed(new Runnable() {
							@Override
							public void run() {//如果未绑定在点击个人空间的时候直接弹出未绑定对话框
								// TODO Auto-generated method stub
								    bindPhone("未绑定");
							}
						}, 500);
	...
	public void onClick(View v) {
		...
		case R.id.btn_loginout:
			...
			dUtil.showDialog("是否注销登录？", 0, "确定", "取消",
	...
	private void bindPhone(String bindInfo)
	{
		if(bindInfo.equals("未绑定"))
		{
		MobileCodeManager smsManager=new MobileCodeManager(mActivity, mActivity,this);
		smsManager.bindMobile();
		}else if(bindInfo.equals("已绑定")){
		DialogUtil info = new DialogUtil(mActivity);
		info.bindMobileInfoShowDialog("提示", "已绑定手机号："+userinfo.getBindMobile(), "确定",
				new DialogButtonClickCallback(){
					@Override 
					public void positiveButtonClick() {}
					@Override
